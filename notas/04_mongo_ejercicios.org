...
db.restaurants.find({"grades.score": {$mod: [7, 0]}}, {_id: 1, name: 1, "grades.score": 1})
...
db.restaurants.find({name: { $regex: /mon/i }},{_id: 1, name: 1, borough:1, "address.coord":1, cuisine:1});
...
db.collection.aggregate([
    {$match: {"entities.hashtags": {$exists: true}}},
...
db.collection.aggregate([
    {$match: {"entities.urls": {$exists: true}}},
    {$group: {_id: "$lang", count: {$sum: 1}}},
    {$sort: {count: -1}},
    {$limit: 10}
])
...
db.collection.aggregate([
    {$match: {"entities.user_mentions": {$exists: true}}},
    {$group: {_id: "$lang", count: {$sum: 1}}},
    {$sort: {count: -1}},
    {$limit: 10}
])
